...
def four_point_transform(image, pts):
    ...
    warped = cv2.warpPerspective(image, warp_matrix, (maxWidth, maxHeight))
    ...
    #     [3643 - 1, 0],
    #     [3643 - 1, 2041 - 1],
    #     [0, 2041 - 1]], dtype="float32")
    # warp_matrix = cv2.perspectiveTransform(rect, dst)
    # warped = cv2.warpPerspective(image, warp_matrix, (3643, 2041))
    # return warped


def main(argv):
    default_file = 'test_image2.jpg'
    filename = argv[0] if len(argv) > 0 else default_file
    image = cv2.imread(filename)
    # load the image, clone it, and initialize the 4 points
    # that correspond to the 4 corners of the chessboard
    pts = np.array([(0+12, 1000), (3643-12, 1000), (3643+int(7000*1.08), 2041), (0-7000, 2041)])
